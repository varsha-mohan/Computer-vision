...
def LucasKanadeInverseCompositional(It,It1,rect):
    ...
    steepest_desc = np.dot(grad,jacobian)
...
if __name__ == '__main__':
    # Load the data
    carseq = sio.loadmat('carseq.mat')
    
    frame_data = carseq['frames']
    frames = frame_data.shape[2]
    
    # Initialise Rects matrix
    rects = np.zeros((frames,4))
    rects[0,:] = np.array([[59,116,145,151]])
    ...
    for x in range(0,frames-1):
        ...
        [u, v] = LucasKanadeInverseCompositional(It,It1,rects[x,:].astype(int))
    ...
    # Plot images with tracker at each frame
    i = [2 ,100 ,200 ,300 ,400]
    count = 1
    a = plt.figure()
    for j in i:
